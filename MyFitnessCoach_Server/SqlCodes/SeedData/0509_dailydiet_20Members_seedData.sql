-- =============================================
-- 0509_dailydiet_20Members_seedData.sql
-- 壓力測試：20 位會員的 DailyDiets 資料
-- Database: MyFitnessCoachDb
--
-- 預估筆數：279,240 筆  (20 members × 6 records/day × 2,327 days)
-- 預估執行時間：15 ~ 45 秒
-- =============================================
...
PRINT '=== DailyDiets 已清空，開始產生 20 位會員資料 ===';
GO

-- ============================================================
-- INSERT：Members 1~20，每天 3 餐 × 2 食物 = 6 筆/人/天
-- 日期範圍：2020-01-01 ~ 2026-05-16（2,327 天）
--
-- 份量計算邏輯：
--   1. 基礎量按體重比例縮放（參考體重 58kg = MemberId=1）
--   2. 長期趨勢按 HealthPlan 調整：
--      維持體重 → 份量微降（×1.0）
--      減重     → 份量明顯降低（×1.8）
--      增肌     → 份量微升（×-0.3，反向）
--   3. 季節波動 + 週波動各加入 member-specific 相位偏移
--   4. 最低保底 20 克/ml
-- ============================================================

;WITH
-- 無遞迴的日期產生器（0~9999 的數字表，取前 2327 天）
...
-- 食物模板（以 58kg 為基準的份量參數）
FoodTpl AS (
    SELECT * FROM (VALUES
    --  meal      fid  msr    base_amt  trnd    seas  wave
...
CROSS JOIN (
    SELECT
        [Id],
        COALESCE([StartWeight], 65.0) / 58.0 AS wf,       -- 體重縮放因子
        CASE
            WHEN [HealthPlan] = N'減重' THEN 1.8
            WHEN [HealthPlan] = N'增肌' THEN -0.3
            ELSE 1.0
        END AS trendMul                                     -- 趨勢方向倍率
    FROM [dbo].[Members]
...
PRINT '=== 0509_dailydiet_20Members_seedData.sql 執行完畢 ===';
GO
